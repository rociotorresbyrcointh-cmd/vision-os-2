        ...
        {SLOTS.map((s, i) => (
          <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
            <div ref={(el) => { refs.current[i] = el }} style={frame}>
              {s.type === 'cover' ? (
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '0 34px' }}>
                  <div style={{ fontFamily: "'Orbitron', sans-serif", fontWeight: 900, fontSize: 26, letterSpacing: '0.05em', marginBottom: 16 }}>VISION<span style={{ color: '#eda45f' }}> OS</span></div>
                  <h2 style={{ fontSize: 28, fontWeight: 900, margin: 0, lineHeight: 1.12 }}>Mirá la app <span style={{ color: '#eda45f' }}>por dentro</span></h2>
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '0 34px' }}>
                  ...
                  <h2 style={{ fontSize: 27, fontWeight: 900, margin: 0, lineHeight: 1.12 }}>Probala <span style={{ color: '#eda45f' }}>gratis</span></h2>
                  <p style={{ margin: '14px 0 20px', fontSize: 14.5, color: 'rgba(255,255,255,0.62)' }}>Sin tarjeta. Sin compromiso.</p>
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#e5883e', borderRadius: 11, padding: '11px 18px', fontSize: 15, fontWeight: 800 }}>visionturnos.online <ArrowRight size={16} /></div>
                </div>
              ) : (
                <>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button onClick={() => download(i)} disabled={s.type === 'shot' && !imgs[i]} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: s.type !== 'shot' || imgs[i] ? '#e5883e' : 'rgba(255,255,255,0.06)', border: 'none', color: 'white', borderRadius: 9, padding: '8px 14px', fontSize: 13, fontWeight: 800, cursor: s.type !== 'shot' || imgs[i] ? 'pointer' : 'not-allowed', opacity: s.type !== 'shot' || imgs[i] ? 1 : 0.5 }}>
