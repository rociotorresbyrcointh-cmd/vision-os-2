...
const slides: DeckSlide[] = [
  { accent: '#e5883e', render: () => (
    <div style={{ textAlign: 'center', padding: '0 34px' }}>
      ...
      <h2 style={{ fontSize: 28, fontWeight: 900, margin: 0, lineHeight: 1.15 }}>Cómo funciona la <span style={{ color: '#eda45f' }}>agenda</span></h2>
      <p style={{ margin: '14px 0 0', fontSize: 16, color: 'rgba(255,255,255,0.6)' }}>Deslizá y mirá lo fácil que es 👉</p>
    </div>
  ) },
  { accent: '#e5883e', render: () => (
    <Feature icon={Calendar} color="#eda45f" title="Cargá un turno en segundos" points={['Elegís cliente, servicio y profesional', 'La duración se calcula sola', 'Queda ordenado en tu agenda']} />
  ) },
  { accent: '#e5883e', render: () => (
    <Feature icon={Repeat} color="#eda45f" title="Turnos recurrentes" points={['Ej: “todos los martes a las 15”', 'La app crea la serie sola', 'Saltea feriados y bloqueos']} />
  ) },
  ...
  { accent: '#e5883e', render: () => (
    <div style={{ textAlign: 'center', padding: '0 34px' }}>
      ...
      <h2 style={{ fontSize: 28, fontWeight: 900, margin: 0, lineHeight: 1.15 }}>Probá la agenda <span style={{ color: '#eda45f' }}>gratis</span></h2>
