  ...
  return (
    <div style={{ width: 168, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.14)', borderRadius: 14, padding: 12, boxShadow: '0 16px 40px rgba(0,0,0,0.4)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 3.5 }}>
        {cells.map((c) => (
          <div key={c} style={{ height: 15, borderRadius: 4, background: c === active ? accent : 'rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, fontWeight: 700, color: c === active ? '#0b0f17' : 'rgba(255,255,255,0.4)' }}>{c > 6 ? c - 6 : ''}</div>
        ))}
      </div>
    </div>
  )
...
const slides: DeckSlide[] = [
  ...
  { accent: '#e5883e', render: () => (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '0 30px 44px' }}>
      <div style={{ fontFamily: "'Orbitron', sans-serif", fontSize: 10, fontWeight: 800, letterSpacing: '0.16em', color: '#eda45f', border: '1px solid rgba(229,136,62,0.4)', borderRadius: 999, padding: '5px 12px', marginBottom: 14 }}>BUSCO NEGOCIOS DE TURNOS</div>
      <MiniCal accent="#e5883e" />
      <h2 style={{ fontSize: 23, fontWeight: 900, margin: '16px 0 0', lineHeight: 1.15, maxWidth: 270 }}>Probá mi app <span style={{ color: '#eda45f' }}>gratis</span> 14 días</h2>
  ...
  { accent: '#e5883e', render: () => (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '0 28px 44px' }}>
      ...
      <div style={{ margin: '14px 0', width: 104, height: 132, borderRadius: 18, border: '2px solid rgba(255,255,255,0.18)', background: 'linear-gradient(160deg, #12122a, #0b0f17)', padding: 9, boxShadow: '0 18px 44px rgba(124,58,237,0.35)' }}>
        <div style={{ fontFamily: "'Orbitron', sans-serif", fontSize: 8, fontWeight: 800, color: '#eda45f', marginBottom: 6 }}>VISION OS</div>
        {[0, 1, 2].map((r) => (
          <div key={r} style={{ display: 'flex', gap: 4, marginBottom: 5, alignItems: 'center' }}>
            <div style={{ width: 5, height: 5, borderRadius: 999, background: ['#eda45f', '#6ee7b7', '#eda45f'][r] }} />
            <div style={{ flex: 1, height: 11, borderRadius: 4, background: 'rgba(255,255,255,0.09)' }} />
          </div>
        ))}
        <div style={{ marginTop: 7, height: 20, borderRadius: 6, background: '#e5883e', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, fontWeight: 800 }}>Nuevo turno</div>
      </div>
      <div style={{ display: 'inline-block', fontSize: 11, fontWeight: 800, color: '#eda45f', border: '1px solid rgba(229,136,62,0.4)', borderRadius: 999, padding: '5px 12px', marginBottom: 8 }}>PROBALA GRATIS 14 DÍAS</div>
